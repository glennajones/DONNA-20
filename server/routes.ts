...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin Daily Email Routes
  app.get("/api/admin/daily-email/test", authenticateToken, async (req: any, res) => {
    try {
  ...
  app.post("/api/admin/daily-email/trigger", authenticateToken, async (req: any, res) => {
    try {
      if (!req.user || req.user.role !== 'admin') {
        return res.status(403).json({ message: 'Admin access required' });
      }
